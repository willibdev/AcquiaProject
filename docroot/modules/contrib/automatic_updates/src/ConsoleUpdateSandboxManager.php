 ...
 *   class.
 */
class ConsoleUpdateSandboxManager extends UpdateSandboxManager {
  ...
  public function performUpdate(): bool {
    ...
    if (!$this->isAvailable()) {
      ...
      if (!$project_info->isInstalledVersionSafe() && $this->isApplying()) {
        $this->logger->notice(
          ...
          [
            '%url' => Url::fromRoute('automatic_updates.update_form')->setAbsolute()->toString(),
          ],
        );
        return $update_started;
      }
    ...
    try {
      ...
      if ($e instanceof SandboxEventException && $e->event instanceof PreCreateEvent) {
      ...
      if (!$e instanceof SandboxEventException) {
      ...
      if ($e instanceof FailureMarkerExistsException || $e instanceof ApplyFailedException) {
