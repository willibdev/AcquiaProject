...
class ReleaseChooserTest extends AutomaticUpdatesKernelTestBase {
  ...
  protected function setUp(): void {
    ...
    $this->setReleaseMetadata([
      'drupal' => static::getDrupalRoot() . '/core/modules/package_manager/tests/fixtures/release-history/drupal.9.8.2-older-sec-release.xml',
    ]);
  }
  ...
  public function testReleases(string $sandbox_manager, bool $minor_support, string $installed_version, ?string $current_minor, ?string $next_minor): void {
    ...
    $sandbox_manager = $this->container->get($sandbox_manager);
    $this->assertReleaseVersion($current_minor, $chooser->getLatestInInstalledMinor($sandbox_manager));
    $this->assertReleaseVersion($next_minor, $chooser->getLatestInNextMinor($sandbox_manager));

    $this->assertReleaseVersion($current_minor, $chooser->getMostRecentReleaseInMinor($sandbox_manager, $this->getRelativeVersion($installed_version, 0)));
    $next_minor_version = $this->getRelativeVersion($installed_version, 1);
    $this->assertReleaseVersion($next_minor, $chooser->getMostRecentReleaseInMinor($sandbox_manager, $next_minor_version));
    ...
    $this->assertReleaseVersion(NULL, $chooser->getMostRecentReleaseInMinor($sandbox_manager, $previous_minor_version));
  }
