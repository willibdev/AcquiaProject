...
/**
 * The app's session banner, slotted into the SDK's <DraftSession>
 * component (registered by @drupal-canvas/headless-nuxt). The SDK owns the
 * session lifecycle (expiry timing, the renewal protocol with the
 * embedding host, re-arming after a renewal) and the visibility of the
 * marked views; this component owns presentation:
 *
 ...
 *   the protocol.
 *
 * Exiting draft mode is a POST form, not a link: exiting changes state
 * (it clears the session cookies), and a GET link would be eligible for
 * prefetching, which could end the session without a click. A plain form
 * needs no JavaScript and nothing prefetches it.
 */
</script>
