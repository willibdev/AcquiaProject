...
trait DurationOptionsTrait {
   ...
   */
  protected function durationOptions(array &$element, FormStateInterface $form_state, array $settings): void {
    $name = $this->getName();
    $form_state_key = $name . '_duration_options';
    $granularity_date = 'days';
    $granularity_datetime = 'days:hours:minutes';
    $granularity = $granularity_datetime;
    if ($this->getDataType() === 'daterange' && $this->getDatetimeType() !== 'datetime') {
      $granularity = $granularity_date;
    }
    if (!$form_state->get($form_state_key)) {
      $form_state->set($form_state_key, $settings['duration_options'] ?? []);
    }

    $allowed_values = $form_state->get($form_state_key);
    $wrapper_id = $name . '-duration-options-wrapper';

    $element['duration_options'] = [
      '#type' => 'fieldset',
      '#title' => $this->t('Duration options'),
    ...
    $element['duration_options']['table'] = [
      '#type' => 'table',
      '#header' => [
        '',
        $this->t('@duration', [
          '@duration' => $granularity === $granularity_datetime ? 'Duration' : 'Days',
        ]),
        $this->t('Label'),
        $this->t('Delete'),
        $this->t('Weight'),
    ...
    if (empty($allowed_values)) {
      $element['duration_options']['table']['#header'] = [
        $this->t('Items'),
      ];
    }

    foreach ($allowed_values as $delta => $allowed_value) {
      $element['duration_options']['table'][$delta] = [
        '#weight' => $delta,
      ];
      // Draggable rows have issues on empty value rows.
      if (!empty($allowed_value['key'])) {
        $element['duration_options']['table'][$delta]['#attributes']['class'] = [
          'draggable',
        ];
      }
      $element['duration_options']['table'][$delta]['handle'] = [
        '#type' => 'markup',
        '#markup' => '',
      ];
      $element['duration_options']['table'][$delta]['key'] = [
        ...
        '#required' => TRUE,
        '#duration_granularity' => $granularity,
        '#attributes' => [
          'class' => ['duration-options'],
        ],
      ];
      $element['duration_options']['table'][$delta]['label'] = [
        '#type' => 'textfield',
        '#maxlength' => 255,
        '#title' => $this->t('Label'),
        ...
        '#required' => TRUE,
        '#attributes' => [
          'placeholder' => $this->t('e.g. 30 days'),
        ],
      ];
      if ($granularity === $granularity_date) {
        $element['duration_options']['table'][$delta]['label']['#title_display'] = 'invisible';
      }
      $element['duration_options']['table'][$delta]['delete'] = [
        '#type' => 'submit',
        '#value' => $this->t('Delete'),
      ...
      $element['duration_options']['table'][$delta]['weight'] = [
        '#type' => 'weight',
    ...
    $element['duration_options']['add_more_allowed_values'] = [
      '#type' => 'submit',
  ...
  public static function addMoreSubmit(array $form, FormStateInterface $form_state): void {
    ...
    $input_allowed_values = NestedArray::getValue($user_input, [...$parents, 'duration_options', 'table']);
    ...
    NestedArray::setValue($user_input, [...$parents, 'duration_options', 'table'], array_values($input_allowed_values));
  ...
  public static function addMoreAjax(array $form, FormStateInterface $form_state): AjaxResponse {
    ...
    $focus_input = $element['table'][$last_child]['key']['days']['#name'];
  ...
  public static function deleteSubmit(array $form, FormStateInterface $form_state): void {
    ...
    // Get the input values to retain the ordered positioning.
    $input_allowed_values = NestedArray::getValue($user_input, [...$parents, 'duration_options', 'table']) ?? $form_state->get($form_state_key);
