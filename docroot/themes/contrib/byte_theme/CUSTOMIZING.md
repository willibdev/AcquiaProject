...
cp -R web/themes/contrib/byte_theme web/themes/custom/byte_theme
git add web/themes/custom/byte_theme
composer remove drupal/byte_theme
```
...
All the objects created this way will be stored in a global variable so you can do stuff with them later. Since the `namespace` variable at the top of component.js is `byte_themeComponents`, you would find the Accordion's ComponentType instance at `window.byte_themeComponents.accordion`.

Furthermore, `window.byte_themeComponents.accordion.instances` is an array of all the ComponentInstance objects, and `window.byte_themeComponents.accordion.elements` is an array of all the component container elements.
