# Diagnosis Theme - Agent Rules

This document contains coding rules and conventions for the Diagnosis theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in Diagnosis theme components.
...
**Rule**: After completing any changes to the Diagnosis theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the Diagnosis theme directory. Running these commands ensures that:
